...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addRoleOption((option) =>
      option
        .setName("role8")
        .setDescription("Selet role for the eighth button")
        .setRequired(true)
    )
    .addStringOption((option) =>
      option
        .setName("emoji8")
        .setDescription("Emoji for the eighth button")
        .setRequired(true)
    )
    .addStringOption((option) =>
      option
        .setName("description8")
        .setDescription("Description for the eighth button")
        .setRequired(true)
    ),
  async execute(interaction) {
    ...
    const description8 = interaction.options.getString("description8");
    ...
    collector.on("collect", async (i) => {
      ...
      if (i.guild.members.me.roles.highest.position < role8.position) {
        return i.update({
          content: `I do not have permission to give you the role ${role8.name}`,
          embeds: [],
          components: [],
        });
      }
      ...
      if (i.customId === "button8") {
        if (i.member.roles.cache.has(role8.id)) {
          await i.member.roles.remove(role8);
          return i.reply({
            content: `You have been removed from the role ${role8.name}`,
            embeds: [],
            components: [],
            ephemeral: true,
          });
        } else {
          await i.member.roles.add(role8);
          return i.reply({
            content: `You have been added to the role ${role8.name}`,
            embeds: [],
            components: [],
            ephemeral: true,
          });
